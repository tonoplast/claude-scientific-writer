...
class ResearchLookup:
    ...
    def _format_papers(self, papers: List[Dict], source: str) -> tuple:
        ...
        lines = [f"Source: {source} (free fallback — no API key set)\n", "=" * 60]
        citations = []
    ...
    def _fallback_lookup(self, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"Semantic Scholar failed ({e}), trying PubMed...", file=sys.stderr)
        ...
        try:
            ...
            print(f"PubMed also failed: {e}", file=sys.stderr)

        return {
            "success": False,
            "query": query,
            "error": "All free fallbacks failed (Semantic Scholar + PubMed). Set PARALLEL_API_KEY or OPENROUTER_API_KEY.",
            "timestamp": timestamp,
            "model": "fallback"
        }
